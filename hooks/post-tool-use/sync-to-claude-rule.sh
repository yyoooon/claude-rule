#!/bin/sh
# Mirror ~/.claude/{CLAUDE.md,skills/,settings.json,hooks/} to ~/Desktop/claude_rule
...
case "$F" in
  "$HOME/.claude/CLAUDE.md"|"$HOME/.claude/skills/"*|"$HOME/.claude/settings.json"|"$HOME/.claude/hooks/"*) ;;
  *) exit 0 ;;
esac
...
cd "$REPO" || exit 0
git add CLAUDE.md skills settings.json hooks >/dev/null 2>&1 || true
