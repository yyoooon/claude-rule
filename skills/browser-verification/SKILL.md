...
### Navigation Check (diff 패턴 산출 후 추가 체크)

diff 패턴 테이블로 cat set을 산출한 뒤, 아래 질문을 반드시 확인:

> **타겟 컴포넌트에 도달하기 위해 click / step 이동이 1번 이상 필요한가?**
> → 필요하면 **cat 3 추가** (diff에 폼 로직 변경이 없어도)

**예시 — cat 3가 추가되는 케이스:**
- 설문 3단계 뒤에 있는 HealthGoalsStep (텍스트·칩 변경만 해도, 이동이 필요하므로 cat 3)
- 모달 내부 컴포넌트 (모달 오픈 클릭이 선행되어야 하므로 cat 3)
- 탭/어코디언 안에 있는 컴포넌트 (탭 클릭이 선행되어야 하므로 cat 3)

cat 3 포함 시 다단계 클릭은 **IIFE 1콜에 묶기** — `click → sleep → click → return`. 절대 eval 여러 번으로 쪼개지 말 것.
